...
- Added Advanced Accessories: a new standalone accessory type where one
  button press sends multiple signals in sequence, with a configurable
  timeout between them. An optional Off signal gives it real on/off state
  (a normal Switch); without one, it's an auto-resetting momentary trigger
